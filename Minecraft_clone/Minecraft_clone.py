...
ground_texture = load_texture('assets/grass_block.png')
...
class Voxel(Button):
    def __init__(self, position = (0, 0, 0), texture= ground_texture):
        super().__init__(
            parent = scene,
            position = position,
            model= '/assets/block',
    ...
    def input(self, key):
        if self.hovered:
            if key == 'left mouse down':
                if block_pick == 1: 
                    voxel = Voxel(position= self.position + mouse.normal, texture = ground_texture)
                if block_pick == 2: 
